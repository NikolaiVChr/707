...
####################### LOOP FOR FUEL WARNING #####################################

var check_tank_warning = func{
	var fuelWarning = 0;
	var warningLevel = 300;
	var textWarning = "FUEL WARNING: Main Tank No. %d has only: %.2fkg left!";
	var textHelp = "Need help? - Use tutorial!";

	if(getprop("consumables/fuel/tank[1]/level-kg") < warningLevel){
		screen.log.write(sprintf(textWarning,4,getprop("consumables/fuel/tank[1]/level-kg")), 1, 0, 0);
		fuelWarning = 1;
	}
	if(getprop("consumables/fuel/tank[2]/level-kg") < warningLevel){
		screen.log.write(sprintf(textWarning,3,getprop("consumables/fuel/tank[2]/level-kg")), 1, 0, 0);
		fuelWarning = 1;
	}
	if(getprop("consumables/fuel/tank[4]/level-kg") < warningLevel){
		screen.log.write(sprintf(textWarning,2,getprop("consumables/fuel/tank[4]/level-kg")), 1, 0, 0);
		fuelWarning = 1;
	}
	if(getprop("consumables/fuel/tank[5]/level-kg") < warningLevel){
		screen.log.write(sprintf(textWarning,1,getprop("consumables/fuel/tank[5]/level-kg")), 1, 0, 0);
		fuelWarning = 1;
	}
	if(fuelWarning) {
		setprop("b707/warning/fuel", 1);
		screen.log.write(textHelp, 1, 0, 0);
		fuelWarning = 0;
		settimer(func{ setprop("b707/warning/fuel", 0)}, 14.5);
		settimer( check_tank_warning, 120);		# give two minutes to refill tank from reserve tank or center tank
	} else {
		settimer( check_tank_warning, 30);		# check every 30 seconds
	}
}

settimer( check_tank_warning, 60); # start first after 60 sec
